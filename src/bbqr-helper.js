...
function goToStep(step) {
  if (step < 1 || step > 2) return; // simplified: only Step 1 & 2 remain
  ...
  return `
    ...
    <div style="max-width: 1000px; margin: 0 auto;">
      ...
      <div style="display: flex; flex-wrap: wrap; gap: 15px; justify-content: center; max-width: 500px; margin: 0 auto;">
        ...
        <!-- Proceed to Scan button removed -->
      </div>
    </div>
  `;
}
...
function renderStep3() {
  return `
    <div style="max-width: 600px; margin: 0 auto;">
      ...
      <!-- Debug: Last QR Text -->
      <textarea id="qr-debug" readonly
                style="width: 100%; height: 90px; padding: 8px; margin-top: 15px;
                       background: #1e293b; color: #e2e8f0; font-size: 12px;
                       font-family: monospace; border-radius: 6px;
                       border: 1px solid rgba(255,255,255,0.2);"></textarea>
...
function scanLoop() {
  ...
  if (video.readyState === video.HAVE_ENOUGH_DATA) {
    ...
    if (code?.data) {
      ...
      if (debugEl) {
        debugEl.value = qrData;
      }

      if (!state.camera.parts.includes(qrData)) {
        state.camera.parts.push(qrData);
      }
    }
    ...
    // Update scan status with clearer text
    let statusText;
    if (currentLanguage === 'zh') {
      // 中文：已知总帧数时，显示 "一共N"，否则保持旧格式
      if (totalParts !== '?') {
        statusText = `一共${totalParts}，已扫${state.camera.parts.length}`;
      } else {
        statusText = `${state.camera.parts.length}/${totalParts} 已扫描`;
      }
    } else {
      // English：保持原有格式，但更友好
      if (totalParts !== '?') {
        statusText = `scanned ${state.camera.parts.length} of ${totalParts}`;
      } else {
        statusText = `${state.camera.parts.length}/${totalParts} scanned`;
      }
    }
    ...
    try {
      ...
      if (fileType === 'P') {
        ...
        let completeText;
        if (currentLanguage === 'zh') {
          completeText = `已完成 (${state.camera.parts.length}/${state.camera.parts.length})`;
        } else {
          completeText = `${state.camera.parts.length}/${state.camera.parts.length} scan complete`;
        }
    ...
    // Fallback: handle single-frame non-BBQr PSBT/transaction
    try {
      const det = detectFileType(qrData);
      if (det && det.raw && (det.fileType === 'P' || det.fileType === 'T')) {
        const rawBytes = new Uint8Array(det.raw);
        if (det.fileType === 'P') {
          const base64 = btoa(String.fromCharCode.apply(null, rawBytes));
          state.signedPsbtData = base64;
          scanStatus.textContent = currentLanguage === 'zh' ? '已完成 (1/1)' : '1/1 scan complete';
          stopCameraScan();
          showScanResults(base64);
          return;
        }
        if (det.fileType === 'T') {
          state.finalizedTx = { hex: Array.from(rawBytes).map(b=>b.toString(16).padStart(2,'0')).join(''), txid: '' };
          scanStatus.textContent = currentLanguage === 'zh' ? '已扫描到交易' : 'Transaction scanned';
          stopCameraScan();
          goToStep(4);
          return;
        }
